...
def acq_max(ac, gp, y_max, bounds, random_state, n_warmup=100000, n_iter=250):
    ...
    max_ind = ys.argsort()[::-1][:2]
    x_max = x_tries[max_ind]
    #max_acq = ys.max()
    max_acq = ys[max_ind]
    ...
    for x_try in x_seeds:
        ...
        # 如果随机过程没有找到最大值，则用下降法，找到波动(可能)曲线的局部最优
        ...
        if max_acq is None or -res.fun[0] >= max_acq[1]:
            #x_max = res.x
            #max_acq = -res.fun[0]
            # 用当前最大值替换此前的最大值
            x_max[1] = res.x
            max_acq[1] = -res.fun[0]
            # 保证第一个总是最大值
            if max_acq[1] > max_acq[0]:
                max_acq[0], max_acq[1] = max_acq[1], max_acq[0]
